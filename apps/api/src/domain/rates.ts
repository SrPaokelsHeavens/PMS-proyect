...
export function selectOvertimeRule(room: RatedRoom, hourPlanId?: string | null, date = new Date()) {
  ...
  return candidates
    ...
    .sort((a, b) => {
      const aIndividual = a.roomId === room.id ? 1 : 0;
      const bIndividual = b.roomId === room.id ? 1 : 0;
      const aSpecificity = aIndividual + (a.roomTypeId ? 1 : 0) + (a.dayGroupId ? 1 : 0) + (a.hourPlanId ? 1 : 0);
      const bSpecificity = bIndividual + (b.roomTypeId ? 1 : 0) + (b.dayGroupId ? 1 : 0) + (b.hourPlanId ? 1 : 0);
